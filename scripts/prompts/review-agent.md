...
1. **再利用性** — 対象リポジトリを別のアプリに差し替えても成立するか（shopping-list固有の構成に依存していないか）
...
**判定は共有知識を汚さない方向に倒してください。** 迷ったら却下し、その知見はこのリポジトリの `CLAUDE.md` 等に残すよう促します。この審査はPRのマージ可否とは独立しており、却下を理由に `00.check-user` を付与したりマージを止めたりはしません。
